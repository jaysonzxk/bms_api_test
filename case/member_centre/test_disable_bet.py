"""
author： mask
filename: test_disable_bet.py
datetime： 2021/4/6 14:47 
ide： PyCharm
"""
import unittest
...
from interfaces.disable_bet import disableBet
from common.get_expect_data import get_expect


@ddt
class testDisableBet(unittest.TestCase, disableBet):

    @classmethod
    def setUpClass(cls) -> None:
        cls.log = Log()
        cls.data = disableBet().test_data
        cls.log.info('正在初始化数据库测试数据。。。。。。')
        databaseOperations().select_bet_status()

    @data(*(get_expect('bms_test_data.xlsx', 'memberCentre', 11)))
    def test_disable_bet(self, test_data):
        ...
        self.log.info('请求接口:{}'.format(disableBet().url))

...
class testMemberisOperateList(unittest.TestCase, memberList):
    ...
    @data(*(get_expect('bms_test_data.xlsx', 'memberCentre', 0, keyword='user_sum', operate=0)))
    def test_member_isOperate_list(self, test_data):
        """
        测试获取会员列表接口
        :param test_data:
        :return:
        """
        res = memberList().get_member_isOperate_list()
